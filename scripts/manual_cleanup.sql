...
BEGIN
    ...
    FOR v_cfg IN
        ...
        FROM (
            VALUES
                ('past_performances_v2', 'race_date ASC, id ASC'),
                ('odds_history', 'recorded_at ASC, id ASC'),
                ('odds_collection_log', 'run_at ASC, id ASC'),
                ('horse_scores_cache', 'id ASC'),
                ('race_scores_cache', 'race_date ASC, id ASC'),
                ('race_runners_clean', 'race_date ASC, id ASC'),
                ('race_runners_scores', 'race_date ASC, id ASC'),
                ('races', 'race_date ASC, id ASC'),
                ('race_runners', 'runner_id ASC'),
                ('horses', 'hkjc_id ASC'),
                ('horses_v2', 'horse_id ASC'),
                ('incident_llm_cache', 'created_at ASC, id ASC'),
                ('jockeys', 'jockey_id ASC'),
                ('trainers', 'trainer_id ASC')
        ) AS t(table_name, order_sql)
    LOOP
        v_row := public.trim_table_rows(v_cfg.table_name, p_keep, v_cfg.order_sql);
...
COMMENT ON FUNCTION public.manual_cleanup(integer) IS
    'Trim append-only/cache tables to p_keep rows (default 20000), deleting oldest rows first.';
