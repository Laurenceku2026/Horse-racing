...
- 无 streamlit 依赖，便于 Streamlit Cloud 导入
"""
...
def fetch_win_odds_snapshot(
    ...
    try:
        url = (
            ...
            f"&odds_type=eq.WIN&select=horse_no,odds_value,recorded_at"
...
def fetch_qin_odds_snapshot(
    ...
    try:
        url = (
            f"{supabase_url}/rest/v1/odds_history"
            f"?race_date=eq.{race_date}&venue=eq.{venue}&race_no=eq.{race_no}"
            f"&odds_type=eq.QIN&select=horse_id,combination,odds_value,recorded_at"
            f"&order=recorded_at.desc&limit=2000"
        )
        response = requests.get(url, headers=headers, timeout=30)
        if response.status_code != 200:
            return {}
...
def build_qin_odds_map(
    ...
    db_odds = fetch_qin_odds_snapshot(race_date, venue, race_no, supabase_url, headers)
    if db_odds:
...
class StrategyBacktester:
    def __init__(self, supabase_url: str = "", supabase_headers: Optional[Dict] = None):
        self.supabase_url = supabase_url
        self.supabase_headers = supabase_headers or {}

    def _get_engine(self) -> BettingStrategyEngine:
    ...
    def _sorted_runners_from_ml(self, runners: List[Dict]) -> List[Dict]:
        return sorted(runners, key=lambda x: float(x.get("win_probability") or 0), reverse=True)
    ...
    def evaluate_qin_race(
        ...
        odds_qin, estimated = build_qin_odds_map(
            race_date,
            venue,
            race_no,
            sorted_runners,
    ...
    def build_summary(
        ...
        if not results:
            return BacktestSummary(model_name=model_name, diagnostics=diagnostics)
